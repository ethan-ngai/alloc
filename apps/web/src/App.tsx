...
function ExecutivePriorities({ company, data, onOpenRequests, onOpenForecast }: { company: CompanyConfig; data: WorkspaceData; onOpenRequests: () => void; onOpenForecast: () => void }) {
  ...
  const strategic = company.key === "northstar"
    ? [{ title: utilization < .75 ? "Hold Cloud services envelope" : "Review Cloud services envelope", detail: utilization < .75 ? `Recorded spend is ${Math.round(utilization * 100)}% of the company limit. Keep capacity unchanged until a reviewed commitment supports an increase.` : "Utilization is approaching the company limit; validate the next provider commitment before changing capacity.", direction: utilization < .75 ? "Hold" : "Review" }, { title: "Tighten Travel spend", detail: "Use trip-level review before expanding travel around the field team.", direction: "Reduce" }]
    : company.key === "juniper"
      ? [{ title: "Reset Food cost target", detail: "Protect margin by reviewing food purchasing before the next planning cycle.", direction: "Reduce" }, { title: "Hold Labor plan", detail: "Keep staffing steady until the next operating forecast refresh.", direction: "Hold" }]
      : [{ title: "Protect Raw materials capacity", detail: "Keep supply capacity available for planned line work before adding new commitments.", direction: "Hold" }, { title: "Review Freight allocation", detail: "Use owner review to decide whether freight needs a larger planning envelope.", direction: "Review" }];
  return <section className="executive-priorities" aria-labelledby="priorities-title"><div className="register-heading"><div><h2 id="priorities-title">Strategic proposals</h2><small>Big-picture changes for human review</small></div><span>Scenario guidance</span></div><div>{strategic.map((proposal) => <article key={proposal.title}><span className="proposal-direction">{proposal.direction}</span><div><strong>{proposal.title}</strong><p>{proposal.detail}</p></div><button type="button" onClick={onOpenForecast}>Inspect why</button></article>)}</div><footer>{reviewRequired ? "A separate one-off request is waiting for a human decision." : "No one-off purchase decision is waiting."}<button type="button" onClick={onOpenRequests}>{reviewRequired ? "Open request" : "Requests"}</button></footer></section>;
}
...
function SpendLimitChart({ data, area, onOpenHistory }: { data: WorkspaceData; area: string; onOpenHistory: () => void }) {
  ...
  const postingAmounts = history.map((item) => Number(item.summary.match(/posted (\d+) [A-Z]{3}/)?.[1] ?? 0));
  const totalPosted = postingAmounts.reduce((total, value) => total + value, 0);
  const currentSpend = snapshot.budget.recognized.amountMinor;
  const startingSpend = Math.max(0, currentSpend - totalPosted);
  let runningSpend = startingSpend;
  const points = history.map((item, index) => { runningSpend += postingAmounts[index]; return { item, amount: postingAmounts[index], spend: runningSpend, x: 64 + index * (600 / Math.max(history.length - 1, 1)) }; });
  const limit = Math.max(snapshot.budget.authorized.amountMinor, 1);
  const coordinate = (spend: number) => 262 - Math.min(spend / limit, 1) * 182;
  ...
  return <section className="spend-limit-chart" aria-labelledby="spend-limit-title"><div className="register-heading"><div><h3 id="spend-limit-title">Spend against limit</h3><small>Fixed snapshot of company cap and recorded spend.</small></div><button type="button" onClick={onOpenHistory}>Open {area} history <ArrowRight size={14} /></button></div><div className="chart-key"><span><i className="spend-key" />Recorded spend <strong>{money(snapshot.budget.recognized)}</strong></span><span><i className="limit-key" />Company limit <strong>{money(snapshot.budget.authorized)}</strong></span></div>{history.length ? <svg viewBox="0 0 728 320" role="img" aria-label={`Recorded spend of ${money(snapshot.budget.recognized)} against a company limit of ${money(snapshot.budget.authorized)}`}><line x1="64" x2="680" y1="80" y2="80" className="chart-grid" /><line x1="64" x2="680" y1="171" y2="171" className="chart-grid" /><line x1="64" x2="680" y1="262" y2="262" className="chart-grid" /><line x1="64" x2="680" y1={coordinate(limit)} y2={coordinate(limit)} className="limit-line" /><text x="680" y={coordinate(limit) - 8} textAnchor="end" className="limit-label">Limit</text><polyline points={path} className="spend-line" />{points.map((point) => <circle key={point.item.id} cx={point.x} cy={coordinate(point.spend)} r={point === largest ? 6 : 3} className={point === largest ? "spend-point highlighted" : "spend-point"} />)}{largest && <g className="chart-callout"><path d={`M ${calloutEdge} 104 L ${largest.x} ${coordinate(largest.spend) - 9}`} /><path d={`M ${largest.x} ${coordinate(largest.spend) - 9} L ${largest.x - 8} ${coordinate(largest.spend) - 17} M ${largest.x} ${coordinate(largest.spend) - 9} L ${largest.x + 3} ${coordinate(largest.spend) - 20}`} /><rect x={calloutX} y="42" width="238" height="62" rx="4" /><text x={calloutX + 12} y="65">Largest linked jump</text><text x={calloutX + 12} y="87" className="callout-detail">{money({ amountMinor: largest.amount, currency: snapshot.budget.authorized.currency })} · {vendor}</text></g>}<text x="64" y="294">Start</text><text x="680" y="294" textAnchor="end">Current</text></svg> : <p className="history-empty">No area-tagged entries are available yet. The next recorded posting will appear here.</p>}</section>;
}
...
function InitiativeBoard({ company, data, onOpenRequests, onOpenMemory, onOpenActivity }: { company: CompanyConfig; data: WorkspaceData; onOpenRequests: () => void; onOpenMemory: (area?: string, evidence?: string) => void; onOpenActivity: (focus?: string) => void }) {
  ...
  const utilization = data.budget.authorized.amountMinor ? data.budget.recognized.amountMinor / data.budget.authorized.amountMinor : 0;
  const initiatives = company.key === "northstar"
    ? [{ title: utilization < .75 ? "Hold company trips envelope" : "Review company trips envelope", direction: utilization < .75 ? "Hold" : "Review", area: "Travel", summary: "Keep field visits at the current planning level.", why: `Recorded spend is ${Math.round(utilization * 100)}% of the company limit. Capacity alone is not evidence for more travel; review individual trips before expanding the envelope.` }, { title: utilization < .75 ? "Hold cloud tooling envelope" : "Review cloud tooling envelope", direction: utilization < .75 ? "Hold" : "Review", area: "Cloud services", summary: "Keep cloud services aligned with the current operating plan.", why: `Recorded spend is ${Math.round(utilization * 100)}% of the company limit. Confirm a provider commitment and delivery need before expanding the envelope.` }, { title: "Tighten equipment purchases", direction: "Reduce", area: "Equipment", summary: "Keep field equipment additions deliberate.", why: "Review the next equipment request against the active project before increasing spend." }]
    : company.key === "juniper"
      ? [{ title: "Lower food cost target", direction: "Reduce", area: "Food", summary: "Protect margin through tighter purchasing.", why: "Food cost is a core operating area. Ask the owner to validate the next purchasing plan before changing the target." }, { title: "Hold labor plan", direction: "Hold", area: "Labor", summary: "Keep staffing stable until the next forecast refresh.", why: "Labor changes should follow an owner review and an updated operating forecast." }, { title: "Review utilities allocation", direction: "Review", area: "Utilities", summary: "Confirm the next facilities commitment.", why: "Use the next reviewed request to decide whether utilities need a new planning envelope." }]
      : [{ title: "Increase material buffer", direction: "Increase", area: "Raw materials", summary: "Protect planned line work from supply gaps.", why: "Raw materials underpin the active production plan. Confirm the next supplier commitment with the owner." }, { title: "Review freight budget", direction: "Review", area: "Freight", summary: "Decide whether freight needs more capacity.", why: "Use current delivery commitments and the next owner review before changing the freight plan." }, { title: "Hold lease exposure", direction: "Hold", area: "Equipment leases", summary: "Keep equipment lease growth paused.", why: "Do not expand lease commitments without a human decision on the next project phase." }];
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      setData((current) => {
        ...
        void ingestSyntheticTick(company, tick, current.budget.authorized.currency, occurredAt).catch((reason) => {
          console.warn("Faker delivery was not mirrored to the local API", reason);
        });
