...
const BACKEND_ORGANIZATIONS: Record<CompanyConfig["key"], string> = {
  northstar: "org_northstar",
  juniper: "org_juniper",
  forge: "org_forge",
};

/** Mirrors posting ticks into the real local API without making the demo UI depend on it. */
export async function ingestSyntheticTick(company: CompanyConfig, tick: number, currency: string, occurredAt: string): Promise<void> {
  ...
  const provenance = {
    kind: "synthetic",
    trust: "authoritative",
    sourceInstanceId: `source_${company.key}_faker_stream`,
    sourceObjectId: identity,
    sourceRevision: "1",
    occurredAt,
    observedAt: occurredAt,
  };
  const response = await fetch(`${BACKEND_URL}/v1/organizations/${organizationId}/imports`, {
    method: "POST",
      ...
      payload: {
        ...
        payload: {
          posting: {
            ...
            postingId: identity,
            revision: 1,
            amount: { amountMinor: event.amountMinor, currency },
            occurredAt,
            status: "posted",
            sourceRef: { type: "source_record", id: identity, revision: 1 },
            scopes: [{ type: "organization", id: organizationId }],
            provenance,
          },
        },
      },
