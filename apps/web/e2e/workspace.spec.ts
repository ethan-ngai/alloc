...
test("pending review, memory, forecast, and activity remain operable", async ({ page }) => {
  ...
  await expect(page.getByText("Verified facts", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Planning lens" })).toBeVisible();
...
test("dependency errors explain recovery and do not display cached balances", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "The data service is unavailable." })).toBeVisible();
