...
| **6A** | OpenClaw/Qwen3.8 27B adapter, scoped tools, inference admission | 1A; local runtime/model availability; contract tool server can be used first | 3B + 4A + 5A + actual local model | Hostile/malformed tool proposals; real model→tool authorization and local-only routing; agent investigates evidence and cannot bypass caps |
